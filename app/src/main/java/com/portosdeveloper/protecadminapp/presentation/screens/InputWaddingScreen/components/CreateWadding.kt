...
@Composable
fun CreateWadding(viewModel : InputWaddingViewModel = hiltViewModel()){

    when(val createWaddingResponse = viewModel.createWaddingResponse){
        Response.Loading -> {
            DefaultProgressBar()
        }
        is Response.Success -> {
        ...
        is Response.Failure -> {
            Toast.makeText(LocalContext.current, createWaddingResponse.exception.message  ?: "Error Desconocido", Toast.LENGTH_LONG).show()
        }
        else -> {}
    }
}