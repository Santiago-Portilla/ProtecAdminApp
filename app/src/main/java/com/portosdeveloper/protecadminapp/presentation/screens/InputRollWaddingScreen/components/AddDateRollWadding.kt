...
@Composable
fun AddDateRollWadding(navController: NavHostController, viewModel: InputRollWaddingViewModel = hiltViewModel()){
    when(val addDateRollWaddingResponse = viewModel.addDateRollWaddingResponse) {
        Response.Loading -> {
            DefaultProgressBar()
        }
        is Response.Success -> {
        ...
        is Response.Failure -> {
            Toast.makeText(LocalContext.current, addDateRollWaddingResponse.exception.message ?: "Error Desconocido", Toast.LENGTH_LONG).show()
        }
        else -> {}
    }
