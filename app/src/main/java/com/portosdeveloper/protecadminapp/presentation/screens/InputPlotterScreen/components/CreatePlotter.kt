...
@Composable
fun CreatePlotter(navController:NavHostController, viewModel: InputPlotterViewModel = hiltViewModel()){
    when(val createPlotterResponse = viewModel.createPlotterResponse){
        is Response.Loading -> {
            DefaultProgressBar()
        }
        is Response.Success -> {
        ...
        is Response.Failure ->{
            Toast.makeText(LocalContext.current, createPlotterResponse.exception.message ?: "Error Desconosido", Toast.LENGTH_LONG).show()
        }
