...
@Composable
fun UpdateYarn(viewModel : InputYarnViewModel = hiltViewModel()){

    when(val updateYarnResponse = viewModel.updateYarnResponse){
        Response.Loading -> {
            DefaultProgressBar()
        }
        is Response.Success -> {
        ...
        is Response.Failure -> {
            Toast.makeText(LocalContext.current, updateYarnResponse.exception.message  ?: "Error Desconocido", Toast.LENGTH_LONG).show()
        }
        else -> {}
    }
}