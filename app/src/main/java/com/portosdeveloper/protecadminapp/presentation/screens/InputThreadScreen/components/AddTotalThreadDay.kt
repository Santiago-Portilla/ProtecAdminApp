...
@Composable
fun AddTotalThreadDay(navController: NavHostController, viewModel: InputThreadViewModel = hiltViewModel()){
    when(val addTotalThreadDay = viewModel.addTotalThreadDayResponse) {
        Response.Loading -> {
            DefaultProgressBar()
        }
        is Response.Success -> {
        ...
        is Response.Failure -> {
            Toast.makeText(LocalContext.current, addTotalThreadDay.exception.message ?: "Error Desconocido", Toast.LENGTH_LONG).show()
        }
        else -> {}
    }
